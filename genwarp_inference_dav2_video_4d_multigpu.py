...
def process_one_video(video_file, camera_pose_file, res, output_path, depth_anything, genwarp_nvs):
    ...
    focal_length_x = camera_poses[0][1]
    focal_length_y = camera_poses[0][2]
    principal_point_x = camera_poses[0][3]
    principal_point_y = camera_poses[0][4]

    idx_frame_out_start = len(frames) // 2

    frames_in = frames[0:idx_frame_out_start]
    frames_out = frames[idx_frame_out_start:]
    camera_poses_in = camera_poses[0:idx_frame_out_start]
    camera_poses_out = camera_poses[idx_frame_out_start:]

    for frame_in, frame_out, camera_pose_in, camera_pose_out in tqdm(
        zip(frames_in, frames_out, camera_poses_in, camera_poses_out),
        total=idx_frame_out_start,
